...
2. Setup `$(go env GOPATH)/bin` to your `PATH`.
3. Run `sh install.sh` in Linux/Mac or `install.ps1` in Windows
...
4. Code is formatted and correctly and follows the best practices, we use `Golang CI Lint` with strict setup to check this.
...
1. Copy GitHub workflow file (.github/workflows/ci.yaml) and install scripts to your own repository.
2. You can optionally copy `.golangci.yaml` as well, or else create your own configuration file at the root of your monorepo.
